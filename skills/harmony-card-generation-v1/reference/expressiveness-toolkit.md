...
- Choose colors for the scene, not from the template by default.
...
- Max 2 major translucent blocks in a 160vp card.
- Blocks should support hierarchy, not become competing cards.

## 3. Text Glyph Icons

Local templates use `Text` as compact glyph/icon carriers when no icon asset is required.
...
```json
{"id":"root","component":"Stack","children":["productImage","contentLayer"],"styles":{"width":"100%","height":160,"clip":true}}
...
- Gradient + glow + image + many blocks all in one 160vp card.
