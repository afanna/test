...
def validate(messages: list[dict[str, Any]]) -> tuple[list[str], list[str]]:
    ...
    for idx, message in enumerate(messages, start=1):
        ...
        if len(body_keys) != 1:
            errors.append(f"message {idx}: must contain exactly one GenUI message body, got {body_keys}")
    ...
    if root:
        ...
        if isinstance(styles, dict):
            if styles.get("width") not in ("100%", 160, 320, "160", "320", "160vp", "320vp"):
                warnings.append("root.styles.width should usually be 160 for 2x2, 320 for horizontal 2x4, or '100%' when required by the host")
            if "height" not in styles:
                warnings.append("root.styles.height is missing; both 2x2 and horizontal 2x4 cards use 160")
            elif styles.get("height") not in (160, "160", "160vp"):
                warnings.append("root.styles.height should usually be 160 for both 2x2 and horizontal 2x4 desktop cards")
        else:
